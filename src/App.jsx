...
function mkDeck(){
  ...
  const zoneCards=LETTERS.flatMap(L=>NUMS.flatMap(N=>{
    const key=`${L}${N}`;
    return ZONE_VARIANTS.map(variant=>({
      id:id++,
      key,
      letter:L,
      number:N,
      variant,
      isZone:true,
      ...FIXED_ZONE_EFFECTS[key][variant],
    }));
  }));
  const godCards=[
...
function aiShouldKeepZoneCard(card,ci,players,forced=false){
  ...
  if(card.variant==='pos')return score>-1.5;
  return score>1.1;
}
function applyFx(card,ci,ti,ps,deck,disc,gs,avoidNegative=false,avoidNegativeFor=[],isAI=false){
  ...
  const killPlayer=i=>{
    ...
    if(P[i].godZone?.length){Disc.push(...P[i].godZone);P[i].godZone=[];P[i].godName=null;P[i].godLevel=0;}
  };
  const hurtHP=(i,v)=>{
    if(i==null||!P[i]||P[i].isDead||(avoidNegative&&i===ci)||avoidNegativeFor.includes(i))return;
    const actualDamage=v;
    P[i].hp=clamp(P[i].hp-actualDamage);
    // 检查是否有伤害传导链条
    if(P[i].damageLink&&P[i].damageLink.active&&actualDamage>0){
      const partnerIdx=P[i].damageLink.partner;
      // 传导伤害给链条另一端
      if(P[partnerIdx]&&!P[partnerIdx].isDead){
        // 先断开链条，避免无限循环
        P[i].damageLink.active=false;
        P[partnerIdx].damageLink.active=false;
        // 双方各失去3HP
        const linkDamage=3;
        P[i].hp=clamp(P[i].hp-linkDamage);
        P[partnerIdx].hp=clamp(P[partnerIdx].hp-linkDamage);
        msgs.push(`【两人一绳】绳索断裂！${P[i].name} 和 ${P[partnerIdx].name} 各失去 ${linkDamage} HP`);
        if(P[i].hp<=0)killPlayer(i);
        if(P[partnerIdx].hp<=0)killPlayer(partnerIdx);
      }
    }
    if(P[i].hp<=0)killPlayer(i);
  };
  const hurtSAN=(i,v)=>{if(i==null||!P[i]||P[i].isDead||(avoidNegative&&i===ci)||avoidNegativeFor.includes(i))return;const oldSan=P[i].san;P[i].san=clamp(P[i].san-v);const newSan=P[i].san;if(newSan<6&&oldSan>=6){const inspectionResult=handleInspection(i,{players:P,deck:D,discard:Disc,inspectionDeck:gs?.inspectionDeck||[],inspectionDiscard:gs?.inspectionDiscard||[],sealLooseningCount:gs?.sealLooseningCount||0,houndsOfTindalosActive:gs?.houndsOfTindalosActive||false,houndsOfTindalosTarget:gs?.houndsOfTindalosTarget||null});P=inspectionResult.players;D=inspectionResult.deck;Disc=inspectionResult.discard;}};
  ...
  switch(card.type){
    ...
    case 'selfDamageAdjDamageBoth': 
      ...
      if(!avoidNegative&&!avoidNegativeFor.includes(ci)){
        ...
        adjacent.forEach(i=>{
          if(!avoidNegativeFor.includes(i)){
            dealHP(i,card.adjHpVal||0);
            dealSAN(i,card.adjSanVal||0);
          }
        });
        msgs.push(`${actor.name} 周围的角色各失去 ${card.adjHpVal||0} HP 和 ${card.adjSanVal||0} SAN`);
      }
      break;
    case 'allDamageHPRandomExtra':
      ...
      if(!avoidNegative&&!avoidNegativeFor.includes(ci)){
        // 全体存活角色失去1HP
        P.forEach((p,i)=>{
          if(!p.isDead&&!avoidNegativeFor.includes(i)){
            dealHP(i,card.val);
          }
        });
        msgs.push(`全体存活角色失去 ${card.val} HP`);
        // 随机选择一名存活角色失去1HP
        const alivePlayers=P.map((p,i)=>i).filter(i=>!P[i].isDead);
        if(alivePlayers.length>0){
          const randomTarget=alivePlayers[Math.floor(Math.random()*alivePlayers.length)];
          dealHP(randomTarget,card.val);
    ...
    case 'revealTopCards':
      ...
      if(!avoidNegative&&!avoidNegativeFor.includes(ci)){
        ...
        if(revealedCards.length>0){
          msgs.push(`${actor.name} 展示了牌堆顶的 ${revealedCards.length} 张牌：${revealedCards.map(c=>`[${c.key}]`).join(' ')}`);
          // 分别统计字母和数字的出现次数
          ...
          if(selectableKeys.length>0){
            ...
            if(isAI){
              ...
              // 将4张牌中该编号的牌收入手牌
              const matchedCards=revealedCards.filter(c=>c.key&&(c.key.includes(selectedKey)));
              if(matchedCards.length>0){
                P[ci].hand.push(...matchedCards);
                msgs.push(`${actor.name} 收入了 ${matchedCards.length} 张编号为 ${selectedKey} 的牌`);
                // 剩余的牌放入弃牌堆
                const remainingCards=revealedCards.filter(c=>!c.key||!c.key.includes(selectedKey));
...
function resolveGodEncounterForAI(ci,godCard,P,Disc,forcedConvert){
  ...
  if(forcedConvert&&P[ci].godName&&P[ci].godName!==godKey){
    const oldSan=P[ci].san;P[ci].san=clamp(P[ci].san-1);const newSan=P[ci].san;if(newSan<6&&oldSan>=6){const inspectionResult=handleInspection(ci,{players:P,deck:D,discard:Disc,inspectionDeck:gs?.inspectionDeck||[],inspectionDiscard:gs?.inspectionDiscard||[],sealLooseningCount:gs?.sealLooseningCount||0,houndsOfTindalosActive:gs?.houndsOfTindalosActive||false,houndsOfTindalosTarget:gs?.houndsOfTindalosTarget||null});P=inspectionResult.players;D=inspectionResult.deck;Disc=inspectionResult.discard;}msgs.push(`${P[ci].name} 被迫改信新神，SAN-1`);
  ...
  if(action==='upgrade'){
    ...
    P.forEach((p,i)=>{if(i!==ci&&p.godName===godKey){
      const oldSan=p.san;p.san=clamp(p.san-1);const newSan=p.san;if(newSan<6&&oldSan>=6){const inspectionResult=handleInspection(i,{players:P,deck:D,discard:Disc,inspectionDeck:gs?.inspectionDeck||[],inspectionDiscard:gs?.inspectionDiscard||[],sealLooseningCount:gs?.sealLooseningCount||0,houndsOfTindalosActive:gs?.houndsOfTindalosActive||false,houndsOfTindalosTarget:gs?.houndsOfTindalosTarget||null});P=inspectionResult.players;D=inspectionResult.deck;Disc=inspectionResult.discard;}Disc.push(...p.godZone);p.godZone=[];p.godName=null;p.godLevel=0;
...
function aiHandleGodCard(ci,godCard,P,D,Disc,L){
  ...
  if (P[ci].role === ROLE_CULTIST) {
    ...
    const oldSan=P[ci].san;P[ci].san=clamp(P[ci].san-sanCost);const newSan=P[ci].san;if(newSan<6&&oldSan>=6){const inspectionResult=handleInspection(ci,{players:P,deck:D,discard:Disc,inspectionDeck:gs?.inspectionDeck||[],inspectionDiscard:gs?.inspectionDiscard||[],sealLooseningCount:gs?.sealLooseningCount||0,houndsOfTindalosActive:gs?.houndsOfTindalosActive||false,houndsOfTindalosTarget:gs?.houndsOfTindalosTarget||null});P=inspectionResult.players;D=inspectionResult.deck;Disc=inspectionResult.discard;}
...
function handleCardDraw(ci, ps, deck, disc, isAI = false, gs = {}) {
  ...
  if (drawnCard.isGod) {
    ...
    if (isAI) {
      ...
      // AI处理邪神牌时，仍然立即扣减SAN值
      if (P[ci].role !== ROLE_CULTIST) {
        const oldSan=P[ci].san;P[ci].san = clamp(P[ci].san - cost);const newSan=P[ci].san;if(newSan<6&&oldSan>=6){const inspectionResult=handleInspection(ci,{players:P,deck:D,discard:Disc,inspectionDeck:gs?.inspectionDeck||[],inspectionDiscard:gs?.inspectionDiscard||[],sealLooseningCount:gs?.sealLooseningCount||0,houndsOfTindalosActive:gs?.houndsOfTindalosActive||false,houndsOfTindalosTarget:gs?.houndsOfTindalosTarget||null});P=inspectionResult.players;D=inspectionResult.deck;Disc=inspectionResult.discard;}
      }
  ...
  if (drawnCard.type === 'caveDuel' && P[ci].hand.length === 0) {
    ...
    const logMsg = `${whoName} 摸到 [${drawnCard.key}] ${drawnCard.name}，之前没有牌，强制展示！`;
  ...
  if (isAI) {
    ...
    if (!keep) {
      Disc.push(drawnCard);
      return { P, D, Disc, drawnCard, effectMsgs: [`${P[ci].name} 摸到 [${drawnCard.key}] ${drawnCard.name}，评估后选择弃置`], needsDecision: false, _aiDrawnCard: drawnCard };
    }
    ...
    const isNegativeEffect = drawnCard.variant === 'negS' || drawnCard.variant === 'negA';
    ...
    if (isTreasureHunter && isNegativeEffect) {
      ...
      if (dodgeSuccess) {
        ...
        P = res.P; D = res.D; Disc = res.Disc; P[ci].hand.push(drawnCard);
        return { P, D, Disc, drawnCard, effectMsgs: [`${P[ci].name}（寻宝者）摸到 [${drawnCard.key}] ${drawnCard.name}，掷出 ${d1} 点，成功规避负面效果！`, ...res.msgs], statePatch: res.statePatch, kept: true, needsDecision: false, _aiDrawnCard: drawnCard };
      }
    }
    ...
    const res = applyFx(drawnCard, ci, null, P, D, Disc, gs, false, [], isAI);
    P = res.P; D = res.D; Disc = res.Disc; P[ci].hand.push(drawnCard);
    return { P, D, Disc, drawnCard, effectMsgs: [`${P[ci].name} 摸到 [${drawnCard.key}] ${drawnCard.name}，选择收入手牌并触发效果`, ...res.msgs], statePatch: res.statePatch, kept: true, needsDecision: false, _aiDrawnCard: drawnCard };
  }
...
function startNextTurn(gs){
    ...
    if(shouldExpire){
      ...
      if(p.damageLink.active){
        ...
        if(P[partnerIdx]&&!P[partnerIdx].isDead){
          ...
          L.push(`【两人一绳】绳索未断裂！${P[i].name} 和 ${P[partnerIdx].name} 各回复 ${healAmount} HP`);
        }
      }
      delete p.damageLink;
    }
  ...
  if(P[next].isResting){
    ...
    if(P[next].godName==='CTH'&&P[next].godLevel>=1){
      ...
      const whoName=next===0?'你':P[next].name;
  ...
  if(next===0){
    ...
    if(!res.drawnCard){L.push('牌堆耗尽！');return{...gs,players:P,deck:D,discard:Disc,log:L,currentTurn:0,phase:'ACTION',drawReveal:null,abilityData:{},skillUsed:false,restUsed:false,huntAbandoned:[],godFromHandUsed:false,godTriggeredThisTurn:false,globalOnlySwapOwner,turn:newTurn};}
    if(res.needGodChoice){return{...gs,players:P,deck:D,discard:Disc,log:[...L,...res.effectMsgs],currentTurn:0,skillUsed:false,restUsed:false,huntAbandoned:[],godFromHandUsed:false,godTriggeredThisTurn:true,phase:'GOD_CHOICE',abilityData:{godCard:res.drawnCard,drawerIdx:0,godEncounterCost:res.godEncounterCost},drawReveal:null,selectedCard:null,globalOnlySwapOwner,_playersBeforeThisDraw:_P_beforeDraw,turn:newTurn};}
    const win=checkWin(P,gs._isMP);if(win)return{...gs,players:P,deck:D,discard:Disc,log:L,gameOver:win,turn:newTurn};
    ...
    if(res.kept){
      return{...gs,players:P,deck:D,discard:Disc,log:[...L,...res.effectMsgs],currentTurn:0,skillUsed:false,restUsed:false,huntAbandoned:[],godFromHandUsed:false,godTriggeredThisTurn:false,
        ...
        selectedCard:null,abilityData:{},globalOnlySwapOwner,_playersBeforeThisDraw:_P_beforeDraw,turn:newTurn,...(res.statePatch||{})};
    }
    return{...gs,players:P,deck:D,discard:Disc,log:L,currentTurn:0,skillUsed:false,restUsed:false,huntAbandoned:[],godFromHandUsed:false,godTriggeredThisTurn:false,
      ...
      selectedCard:null,abilityData:{},globalOnlySwapOwner,_playersBeforeThisDraw:_P_beforeDraw,turn:newTurn};
  }else if(gs._isMP){
    ...
    if(res.kept){
      return{...gs,players:P,deck:D,discard:Disc,log:L,currentTurn:next,skillUsed:false,restUsed:false,huntAbandoned:[],godFromHandUsed:false,godTriggeredThisTurn:false,
        ...
        selectedCard:null,abilityData:{},_isMP:gs._isMP,globalOnlySwapOwner,...(res.statePatch||{})};
    }
    return{...gs,players:P,deck:D,discard:Disc,log:L,currentTurn:next,skillUsed:false,restUsed:false,huntAbandoned:[],godFromHandUsed:false,godTriggeredThisTurn:false,
      phase:'DRAW_REVEAL',
      ...
      selectedCard:null,abilityData:{},_isMP:gs._isMP,globalOnlySwapOwner};
  }else{
    ...
    if(!P[next].isDead&&P[next].role===ROLE_TREASURE&&isWinHand(P[next].hand)){P[next].roleRevealed=true;return{...gs,players:P,deck:D,discard:Disc,log:[...L,`${P[next].name} 集齐全部编号并获胜！`],gameOver:{winner:ROLE_TREASURE,reason:`${P[next].name} 集齐了全部编号并获胜！`,winnerIdx:next}};}
    return{...gs,players:P,deck:D,discard:Disc,log:L,currentTurn:next,phase:'AI_TURN',drawReveal:null,selectedCard:null,abilityData:{...gs.abilityData,...(res.statePatch?.abilityData||{})},huntAbandoned:[],
...
function aiStep(gs){
  ...
  if(abilityData.caveDuelTargets&&abilityData.caveDuelSource===ct){
    ...
    if(validTargets.length>0){
      ...
      if(targetIdx===0){
        // 玩家作为目标角色，需要选择牌
        return{...gs,players:P,deck:D,discard:Disc,log:L,abilityData:{...abilityData,caveDuelTarget:targetIdx,sourceCardIndex:sourceCardIndex,sourceCard:sourceCard},currentTurn:ct,phase:'CAVE_DUEL_SELECT_CARD'};
      }else{
        // AI作为目标角色，选择数字编号最大的牌
        ...
        if(sourceNumber>targetNumber){
          ...
          L.push(`【穴居人战争】${sourcePlayer.name} 亮出 [${sourceCard.key}] ${sourceCard.name}，${targetPlayer.name} 亮出 [${targetCard.key}] ${targetCard.name}，${sourcePlayer.name} 胜出，收下两张牌`);
        }else if(targetNumber>sourceNumber){
          // 目标角色获胜，收下两张牌
          sourcePlayer.hand.splice(sourceCardIndex,1);
          targetPlayer.hand.splice(targetCardIndex,1);
          targetPlayer.hand.push(sourceCard,targetCard);
          L.push(`【穴居人战争】${sourcePlayer.name} 亮出 [${sourceCard.key}] ${sourceCard.name}，${targetPlayer.name} 亮出 [${targetCard.key}] ${targetCard.name}，${targetPlayer.name} 胜出，收下两张牌`);
        }else{
          // 平局，各自收回自己的牌
          L.push(`【穴居人战争】${sourcePlayer.name} 亮出 [${sourceCard.key}] ${sourceCard.name}，${targetPlayer.name} 亮出 [${targetCard.key}] ${targetCard.name}，平局，各自收回自己的牌`);
        }
      }
    }
    // 清除能力数据
    return{...gs,players:P,deck:D,discard:Disc,log:L,abilityData:{},currentTurn:ct,phase:'AI_TURN'};
  }
  ...
  if(!gs.skillUsed&&!gs.restUsed){
    ...
    if(handGodIdx>=0){
      ...
      if(willWorship){
        ...
        if(alreadyHasGod){const oldSan=P[ct].san;P[ct].san=clamp(P[ct].san-1);const newSan=P[ct].san;if(newSan<6&&oldSan>=6){const inspectionResult=handleInspection(ct,{players:P,deck:D,discard:Disc,inspectionDeck:gs?.inspectionDeck||[],inspectionDiscard:gs?.inspectionDiscard||[],sealLooseningCount:gs?.sealLooseningCount||0,houndsOfTindalosActive:gs?.houndsOfTindalosActive||false,houndsOfTindalosTarget:gs?.houndsOfTindalosTarget||null});P=inspectionResult.players;D=inspectionResult.deck;Disc=inspectionResult.discard;}Disc.push(...P[ct].godZone);P[ct].godZone=[];P[ct].godName=null;P[ct].godLevel=0;L.push(`${P[ct].name} 改信新神，SAN-1`);}
        ...
        P.forEach((p,i)=>{if(i!==ct&&p.godName===hgc.godKey){const oldSan=p.san;p.san=clamp(p.san-1);const newSan=p.san;if(newSan<6&&oldSan>=6){const inspectionResult=handleInspection(i,{players:P,deck:D,discard:Disc,inspectionDeck:gs?.inspectionDeck||[],inspectionDiscard:gs?.inspectionDiscard||[],sealLooseningCount:gs?.sealLooseningCount||0,houndsOfTindalosActive:gs?.houndsOfTindalosActive||false,houndsOfTindalosTarget:gs?.houndsOfTindalosTarget||null});P=inspectionResult.players;D=inspectionResult.deck;Disc=inspectionResult.discard;}Disc.push(...p.godZone);p.godZone=[];p.godName=null;p.godLevel=0;L.push(`${p.name} 被邪神抛弃，SAN-1`);}});
  ...
  if(shouldRest){
    ...
    while(P[ct].hand.length>aiHandLimit){const c=P[ct].hand.shift();Disc.push(c);L.push(`${ai.name} 弃 [${c.key}]（上限）`);}
  ...
  if(useSkill){
    ...
    if(aiEffRole===ROLE_HUNTER){
      ...
      if(P[ct].hand.length){
        ...
        L.push(`${ai.name}（邪祀者）对 ${tgt.name} 【蛊惑】，赠予 [${sc.isGod?sc.name:sc.key}]`);
        if(sc.isGod){
...
function handleInspection(playerIndex, gs) {
  ...
  switch (drawnCard.effect) {
    ...
    case 'disableRest': {
      // 下一回合禁用"休息"
      P[playerIndex].disableRest = true;
      L.push(`${P[playerIndex].name} 失眠，下一回合禁用休息`);
      break;
    }
    case 'nothing': {
      // 什么也不做
      L.push(`${P[playerIndex].name} 获得暂时的平静`);
      break;
    }
    case 'flip': {
      // 翻面
    ...
    case 'disableSkill': {
      // 下一回合禁用技能
      P[playerIndex].disableSkill = true;
      L.push(`${P[playerIndex].name} 失忆，下一回合禁用技能`);
      break;
    }
    case 'handLimitDecrease': {
      // 下一回合手牌上限-1
      P[playerIndex].handLimitDecrease = 1;
...
function buildAnimQueue(oldGs,newGs){
  ...
  if(deaths.length){
    ...
    // 检测地动山摇效果
    const earthquakeMsg=newMsgs.find(m=>m.includes('地动山摇'));
    if(earthquakeMsg){
...
function DiscardMoveOverlay({anim,exiting}){
  ...
  React.useEffect(() => {
    ...
    if(targetPid>0){
      ...
      // Find actual hand area position via DOM query
...
export default function Game(){
  ...
  useEffect(()=>{
    ...
    timerRef.current=setTimeout(()=>{
      ...
      if(discardMsg&&rawResult._aiDrawnCard){
        queue.push({type:'DISCARD',card:rawResult._aiDrawnCard,triggerName:gs.players[gs.currentTurn]?.name||'???'});
      }
  ...
  useEffect(()=>{
    if(!gs||gs.phase!=='DAMAGE_LINK_SELECT_TARGET'||gs.gameOver||gs.phase==='PLAYER_WIN_PENDING'||anim||showTutorial||gs._isMP)return;
    const {damageLinkTargets,damageLinkSource}=gs.abilityData;
    if(!damageLinkTargets||!damageLinkSource)return;
    // Check if the source is an AI (not player 0)
    if(damageLinkSource===0)return;
  ...
  const isLocalDrawDecision=!!(
    gs&&gs.phase==='DRAW_REVEAL'&&gs.drawReveal?.needsDecision&&((gs.drawReveal?.drawerIdx??-1)===0)
  );
  const isLocalGodChoice=!!(
    gs&&gs.phase==='GOD_CHOICE'&&gs.abilityData?.godCard&&((gs.abilityData?.drawerIdx??gs.currentTurn)===0)
  );
  const isMpCthDecisionPhase=!!(
    isMultiplayer&&gs&&(
      (gs.phase==='DRAW_REVEAL'&&gs.drawReveal?.needsDecision&&gs.drawReveal?.fromRest&&((gs.drawReveal?.drawerIdx??-1)===0))||
      (gs.phase==='GOD_CHOICE'&&gs.abilityData?.fromRest&&gs.abilityData?.godCard&&((gs.abilityData?.drawerIdx??gs.currentTurn)===0))
    )
  );
  ...
  useEffect(()=>{
    ...
    if(base.phase==='DRAW_REVEAL'&&base.drawReveal?.needsDecision&&base.drawReveal?.fromRest){
      ...
      const newGs={...base,
        ...
        log:[...base.log,`(超时) ${who} 弃置了 [${dr.card.key}] ${dr.card.name}`],
        phase:'ACTION',
        drawReveal:null,
    ...
    function resolveToAction(g){
      ...
      if(phase==='DRAW_REVEAL'){
        ...
        if(dr.needsDecision){
          if(dr.forcedKeep){
            ...
            return{...g,players:P,deck:D,discard:Disc,log:[...g.log,`(超时) ${dr.drawerName||'该玩家'}被迫收入 [${dr.card.key}]`,...res.msgs],phase:'ACTION',drawReveal:null,abilityData:{},...(res.statePatch||{})};
          }
          return{...g,discard:[...g.discard,dr.card],log:[...g.log,`(超时) ${dr.drawerName||'该玩家'}弃置了 [${dr.card.key}] ${dr.card.name}`],phase:'ACTION',drawReveal:null,abilityData:{}};
        }
      ...
      if(phase==='NYA_BORROW'){
        ...
        if(res.needsDecision){
          return{...g,players:P,deck:D,discard:[...Disc,res.drawnCard],log:[...L,`(超时) 弃置了 [${res.drawnCard.key}] ${res.drawnCard.name}`],phase:'ACTION',drawReveal:null,abilityData:{}};
        }
        // 普通牌
...
{isLose?(winner==='LOSE_ALL'?'——  全员覆灭  ——':'英魂殒落'):iWon?'胜利归你':winner===ROLE_TREASURE?`——  ${gs.players[winnerIdx]?.name??''}获胜  ——`:'——  '+winner+'获胜  ——'}
  ...
  function _cthContinueRestDraws(baseGsAfterDecision){
    ...
    // 被跳过的回合（fromRest=true）：不执行弃牌判定，直接进入下一回合
    // 主动结束的回合（fromRest=false）：之前已经执行过弃牌判定，不再触发，直接进入下一回合
    const nextGs=startNextTurn({...baseGsAfterDecision,players:P,deck:D,discard:Disc,log:L,currentTurn:baseGsAfterDecision.currentTurn,abilityData:{}});
    applyNextTurnGs(nextGs);
  }
  ...
  function handleDrawKeep(){
    ...
    if(dr.card.type==='swapAllHands'){
      setGs({...gs,phase:'ZONE_SWAP_SELECT_TARGET',drawReveal:null,abilityData:{zoneSwapCard:dr.card,fromRest:dr.fromRest,cthDrawsRemaining:gs.abilityData?.cthDrawsRemaining},log:[...gs.log,`你摸到 [${dr.card.key}] ${dr.card.name}，请选择交换手牌的目标`]});
      return;
    }
    ...
    const drawerIdx=dr.drawerIdx??0;
    // 当其他角色摸到负面区域牌时，dr.card.variant可能是undefined，所以需要检查卡片类型
    const isAOENegativeEffect=(isNegativeEffect||dr.card.type.includes('Damage')||dr.card.type.includes('damage')||dr.card.type.includes('Discard')||dr.card.type.includes('discard')||dr.card.type.includes('Rest')||dr.card.type.includes('rest'))&&(dr.card.type.startsWith('all')||dr.card.type.startsWith('adj'));
    ...
    if(drawerIdx===0&&!P[0].isDead&&(P[0]._nyaBorrow||P[0].role)==='寻宝者'&&isWinHand(P[0].hand)){
  ...
  function handleTreasureDodge(gs, dr, isAOE = false) {
    ...
    if (dodgeSuccess && !isAOE) {
      L.push(`${who} 收入了 [${dr.card.key}] ${dr.card.name}（负面效果已规避）`, ...res.msgs);
    } else {
  ...
  function handleTreasureDodgeRoll(){
    ...
    const queue=buildAnimQueue(gs,result.newGs);
    // 无论是否有其他动画，都播放骰子动画
    pendingGsRef.current=result.newGs;
  ...
  function handleTreasureAOEDodgeRoll(){
    ...
    if(dr.fromRest&&!result.win){_cthContinueRestDraws(result.newGs);return;}
    const queue=buildAnimQueue(gs,result.newGs);
    // 无论是否有其他动画，都播放骰子动画
    pendingGsRef.current=result.newGs;
    animQueueRef.current=queue;
  ...
  function peekHandSelectTarget(ti){
    ...
    const {peekHandTargets}=gs.abilityData;
    ...
    if(gs._isMP){
      ...
      L=[...gs.log,`${gs.players[0].name} 偷看了 ${targetPlayer.name} 的一张手牌`];
    }else{
      // 单机游戏：显示具体卡牌信息
  ...
  function executeCaveDuel(P, caveDuelSource, ti, sourceCardIndex, targetCardIndex, sourceCard, targetCard, gs){
    ...
    if(sourceNumber>targetNumber){
      ...
      L=[...gs.log,`【穴居人战争】${P[caveDuelSource].name} 亮出 [${sourceCard.key}] ${sourceCard.name}，${P[ti].name} 亮出 [${targetCard.key}] ${targetCard.name}，${P[caveDuelSource].name} 胜出，收下两张牌`];
    }else if(targetNumber>sourceNumber){
      // 目标角色获胜，收下两张牌
      P[caveDuelSource].hand.splice(sourceCardIndex,1);
      P[ti].hand.splice(targetCardIndex,1);
      P[ti].hand.push(sourceCard,targetCard);
      L=[...gs.log,`【穴居人战争】${P[caveDuelSource].name} 亮出 [${sourceCard.key}] ${sourceCard.name}，${P[ti].name} 亮出 [${targetCard.key}] ${targetCard.name}，${P[ti].name} 胜出，收下两张牌`];
    }else{
      // 平局，各自收回自己的牌
      L=[...gs.log,`【穴居人战争】${P[caveDuelSource].name} 亮出 [${sourceCard.key}] ${sourceCard.name}，${P[ti].name} 亮出 [${targetCard.key}] ${targetCard.name}，平局，各自收回自己的牌`];
    }
    const nextGs={...gs,players:P,log:L,phase:'ACTION',abilityData:{
      ...(gs.abilityData?.fromRest?{fromRest:true}:{}),
      ...(gs.abilityData?.cthDrawsRemaining!=null?{cthDrawsRemaining:gs.abilityData.cthDrawsRemaining}:{}),
  ...
  function damageLinkSelectTarget(ti){
    ...
    if(gs.abilityData?.fromRest){_cthContinueRestDraws(nextGs);return;}
    // 触发链条建立的动画，确保特效能够正确显示
    triggerAnimQueue([{type:'CARD_TRANSFER',msgs:L.slice(-1)}],nextGs);
  }
  ...
  function aiChooseRevealCard(targetHand, hunterHand, log, discardPile){
    ...
    // 分析游戏日志和弃牌堆，计算每张区域牌的安全分数
    const scored=zoneCards.map(c=>{
      let safetyScore=0;
      
      // 1. 查看游戏日志里有没有该追猎者收入手牌后，从未进入弃牌堆的区域牌
  ...
  function playerRevealForHunt(cardIdx){
    ...
    while(P[huntingAI].hand.length>aiHandLimit){const c=P[huntingAI].hand.shift();Disc.push(c);L.push(`${aiHunterName} 弃 [${c.key}]（上限）`);}
    ...
    else if (wantsToHuntAgain) newGs = {...baseGs, phase: 'AI_TURN', currentTurn: huntingAI, skillUsed: true};
  ...
  function tortoiseOracleSelect(key){    
    ...
    let P=copyPlayers(gs.players),D=[...gs.deck],Disc=[...gs.discard];
    // 将4张牌中该编号的牌收入手牌
    const matchedCards=revealedCards.filter(c=>c.key&&(c.key.includes(key)));
    const L=[...gs.log,`你选择了编号 ${key}`];
    if(matchedCards.length>0){
      P[playerIndex].hand.push(...matchedCards);
      L.push(`你收入了 ${matchedCards.length} 张编号为 ${key} 的牌`);
      // 剩余的牌放入弃牌堆
      const remainingCards=revealedCards.filter(c=>!c.key||!c.key.includes(key));
      if(remainingCards.length>0){
        Disc.push(...remainingCards);
      }
    }else{
  ...
  function bewitchSelectTarget(ti){
    ...
    if(bewitchCard.isGod){
      ...
      if (P[ti].role === '邪祀者') {
        ...
        const oldSan=P[ti].san;P[ti].san=clamp(P[ti].san-cost);const newSan=P[ti].san;if(newSan<6&&oldSan>=6){const inspectionResult=handleInspection(ti,{players:P,deck:D,discard:Disc,inspectionDeck:gs.inspectionDeck,inspectionDiscard:gs.inspectionDiscard,sealLooseningCount:gs.sealLooseningCount,houndsOfTindalosActive:gs.houndsOfTindalosActive,houndsOfTindalosTarget:gs.houndsOfTindalosTarget});P=inspectionResult.players;D=inspectionResult.deck;Disc=inspectionResult.discard;}
  ...
  function godResolvePlayer(action){
    ...
    if(godEncounterCost>0){
      const oldSan=P[0].san;P[0].san=clamp(P[0].san-godEncounterCost);const newSan=P[0].san;if(newSan<6&&oldSan>=6){const inspectionResult=handleInspection(0,{players:P,deck:D,discard:Disc,inspectionDeck:gs.inspectionDeck,inspectionDiscard:gs.inspectionDiscard,sealLooseningCount:gs.sealLooseningCount,houndsOfTindalosActive:gs.houndsOfTindalosActive,houndsOfTindalosTarget:gs.houndsOfTindalosTarget});P=inspectionResult.players;D=inspectionResult.deck;Disc=inspectionResult.discard;}
    }
    if(action==='keepHand'){
      ...
      if(action==='forcedConvert'||(P[0].godName&&P[0].godName!==gk)){
        const oldSan=P[0].san;P[0].san=clamp(P[0].san-1);const newSan=P[0].san;if(newSan<6&&oldSan>=6){const inspectionResult=handleInspection(0,{players:P,deck:D,discard:Disc,inspectionDeck:gs.inspectionDeck,inspectionDiscard:gs.inspectionDiscard,sealLooseningCount:gs.sealLooseningCount,houndsOfTindalosActive:gs.houndsOfTindalosActive,houndsOfTindalosTarget:gs.houndsOfTindalosTarget});P=inspectionResult.players;D=inspectionResult.deck;Disc=inspectionResult.discard;}Disc.push(...P[0].godZone);P[0].godZone=[];P[0].godName=null;P[0].godLevel=0;
        L.push('改信新神，失去1SAN，旧神牌入弃牌堆');
      }
      ...
      P.forEach((p,i)=>{if(i>0&&p.godName===gk){const oldSan=p.san;p.san=clamp(p.san-1);const newSan=p.san;if(newSan<6&&oldSan>=6){const inspectionResult=handleInspection(i,{players:P,deck:D,discard:Disc,inspectionDeck:gs.inspectionDeck,inspectionDiscard:gs.inspectionDiscard,sealLooseningCount:gs.sealLooseningCount,houndsOfTindalosActive:gs.houndsOfTindalosActive,houndsOfTindalosTarget:gs.houndsOfTindalosTarget});P=inspectionResult.players;D=inspectionResult.deck;Disc=inspectionResult.discard;}Disc.push(...p.godZone);p.godZone=[];p.godName=null;p.godLevel=0;L.push(`${p.name} 被邪神抛弃，SAN-1`);}});
    } else {
  ...
  function confirmDiscard(){
    ...
    if(P[0].isResting&&P[0].godName==='CTH'&&P[0].godLevel>=1){
      ...
      for(let _d=0;_d<extraDraws;_d++){
        ...
        if(r2.drawnCard)L.push(`  摸到 [${r2.drawnCard.key||r2.drawnCard.name}]`);
        if(r2.needGodChoice){
          setGs({...baseGs,players:P,deck:D,discard:Disc,log:L,currentTurn:0,skillUsed:false,restUsed:false,huntAbandoned:[],godFromHandUsed:false,godTriggeredThisTurn:true,phase:'GOD_CHOICE',abilityData:{godCard:r2.drawnCard,fromRest:true,cthDrawsRemaining:extraDraws-_d-1,drawerIdx:0},drawReveal:null,selectedCard:null});
          return;
        }
        if(r2.needsDecision){
  ...
  function doRest(){
    if(phase!=='ACTION'||isBlocked||gs.restUsed||gs.skillUsed||P[0].disableRest)return;
    ...
    if(P[0].isResting&&P[0].godName==='CTH'&&P[0].godLevel>=1){
      ...
      const handleDraws=()=>{
        ...
        for(let _d=0;_d<extraDraws;_d++){
          ...
          if(r2.drawnCard)L.push(`  摸到 [${r2.drawnCard.key||r2.drawnCard.name}]`);
          if(r2.needGodChoice){
            setGs({...oldGs,players:P,deck:D,discard:Disc,log:L,currentTurn:0,skillUsed:true,restUsed:true,huntAbandoned:[],godFromHandUsed:false,godTriggeredThisTurn:true,phase:'GOD_CHOICE',abilityData:{godCard:r2.drawnCard,fromRest:true,cthDrawsRemaining:extraDraws-_d-1,drawerIdx:0},drawReveal:null,selectedCard:null});
            return;
          }
          if(r2.needsDecision){
  ...
  function applyNextTurnGs(newGs){
    ...
    const drawStatQ=newGs?buildAnimQueue({...gs,players:newGs._playersBeforeThisDraw||gs.players},newGs):[];
    ...
    if(newGs.drawReveal?.card&&newGs.phase==='ACTION'){
      ...
      animQueueRef.current=[...drawStatQ];
      setGs({...newGs,phase:'ACTION',drawReveal:null,abilityData:{}});
      setAnim({type:'DRAW_CARD',card:newGs.drawReveal.card,triggerName:drawerName,targetPid:drawerPid});
      return;
    }
    setGs(newGs);
  }
  ...
  function endTurn(){
    ...
    if(P[0].isResting&&P[0].godName==='CTH'&&P[0].godLevel>=1){
      ...
      for(let _d=0;_d<extraDraws;_d++){
        ...
        if(r2.drawnCard){
          L.push(`  摸到 [${r2.drawnCard.key||r2.drawnCard.name}]`);
          cthDraws.push(r2.drawnCard);
        }
        if(r2.needGodChoice){
          setGs({...gs,players:P,deck:D,discard:Disc,log:L,currentTurn:0,skillUsed:false,restUsed:false,huntAbandoned:[],godFromHandUsed:false,godTriggeredThisTurn:true,phase:'GOD_CHOICE',abilityData:{godCard:r2.drawnCard,fromRest:true,cthDrawsRemaining:extraDraws-_d-1,drawerIdx:0},drawReveal:null,selectedCard:null});
          return;
        }
        if(r2.needsDecision){
  ...
  function _onRoleRevealDone(pendingGs){
    ...
    if(pendingGs._isMP&&pendingGs.currentTurn!==0){
      ...
      if(drawnCard){
        ...
        const drawStatQ=buildAnimQueue({...gs,players:pendingGs._playersBeforeThisDraw||gs.players},pendingGs);
        animQueueRef.current=[...drawStatQ];
    ...
    if(pendingGs.drawReveal?.card){
      ...
      const drawStatQ=buildAnimQueue({...gs,players:pendingGs._playersBeforeThisDraw||gs.players},pendingGs);
      triggerAnimQueue([{type:'YOUR_TURN'},{type:'DRAW_CARD',card:pendingGs.drawReveal.card,triggerName:'你'},...drawStatQ],pendingGs);
    }else{
      ...
      if(godCard) queue.push({type:'DRAW_CARD',card:godCard,triggerName:'你'});
      queue.push(...buildAnimQueue({...gs,players:pendingGs._playersBeforeThisDraw||gs.players},pendingGs));
      triggerAnimQueue(queue,pendingGs);
    }
  }
  ...
  const phaseLabel={
    ...
    SWAP_GIVE_CARD:       `${gs.players[gs.abilityData?.swapTi]?.revealHand ? '抽到' : '暗抽到'} [${gs.abilityData?.takenCard?.key}]，选一张手牌还给对方`,
    HUNT_SELECT_TARGET:   '【追捕】选择猎物',
    HUNT_CONFIRM:         myTurn?`[${gs.abilityData?.revCard?.key}] ${gs.abilityData?.revCard?.name} 已亮出！弃出匹配手牌造成3HP，或放弃`:(gs._isMP?'请等待追猎者做出选择…':`[${gs.abilityData?.revCard?.key}] 已亮出`),
    ...
    CAVE_DUEL_SELECT_CARD: '【穴居人战争】选择要亮的手牌',
  }[phase]||'';

  const selectingOther=['SWAP_SELECT_TARGET','HUNT_SELECT_TARGET','BEWITCH_SELECT_TARGET','ZONE_SWAP_SELECT_TARGET','PEEK_HAND_SELECT_TARGET','CAVE_DUEL_SELECT_TARGET','DAMAGE_LINK_SELECT_TARGET','TORTOISE_ORACLE_SELECT'].includes(phase);
  const selectingCardFromPublic=phase==='HUNT_SELECT_CARD_FROM_PUBLIC';
  // 多人游戏中 HUNT_CONFIRM 非追猎者不显示操作按钮区域
  const cancelable=['SWAP_SELECT_TARGET','SWAP_SELECT_TARGET_CARD','SWAP_GIVE_CARD','HUNT_SELECT_TARGET','ZONE_SWAP_SELECT_TARGET','PEEK_HAND_SELECT_TARGET','CAVE_DUEL_SELECT_TARGET','DAMAGE_LINK_SELECT_TARGET','TORTOISE_ORACLE_SELECT',...(phase==='HUNT_CONFIRM'&&gs._isMP&&!myTurn?[]:['HUNT_CONFIRM']),'BEWITCH_SELECT_CARD','BEWITCH_SELECT_TARGET'].includes(phase);
  // In HUNT_CONFIRM, 放弃追捕 replaces ✕取消 — never show both
  const showCancelBtn=cancelable&&phase!=='HUNT_CONFIRM'&&(!gs._isMP||myTurn)&&!anim;
  ...
  function worshipFromHand(idx){
    ...
    if(isUpgrade){
      ...
      const oldSan=P[0].san;P[0].san=clamp(P[0].san-1);const newSan=P[0].san;if(newSan<6&&oldSan>=6){const inspectionResult=handleInspection(0,{players:P,deck:D,discard:Disc,inspectionDeck:gs.inspectionDeck,inspectionDiscard:gs.inspectionDiscard,sealLooseningCount:gs.sealLooseningCount,houndsOfTindalosActive:gs.houndsOfTindalosActive,houndsOfTindalosTarget:gs.houndsOfTindalosTarget});P=inspectionResult.players;D=inspectionResult.deck;Disc=inspectionResult.discard;}Disc.push(...P[0].godZone);
    ...
    P.forEach((p,i)=>{if(i>0&&p.godName===godKey){const oldSan=p.san;p.san=clamp(p.san-1);const newSan=p.san;if(newSan<6&&oldSan>=6){const inspectionResult=handleInspection(i,{players:P,deck:D,discard:Disc,inspectionDeck:gs.inspectionDeck,inspectionDiscard:gs.inspectionDiscard,sealLooseningCount:gs.sealLooseningCount,houndsOfTindalosActive:gs.houndsOfTindalosActive,houndsOfTindalosTarget:gs.houndsOfTindalosTarget});P=inspectionResult.players;D=inspectionResult.deck;Disc=inspectionResult.discard;}Disc.push(...p.godZone);p.godZone=[];p.godName=null;p.godLevel=0;L.push(`${p.name} 被邪神抛弃，SAN-1`);}});
    const win=checkWin(P,gs._isMP);
  ...
  function handleMyCardClick(idx){
    ...
    else if(phase==='CAVE_DUEL_SELECT_CARD'&&myTurn){
  ...
  return(<>
    ...
    animation:deathShake?'deathShakeAnim 2.0s ease-in-out':screenShake?'screenShakeAnim 0.38s ease-in-out':undefined,
      ...
      {/* Draw reveal modal */}
      {!suppressAnim&&phase==='DRAW_REVEAL'&&gs.drawReveal&&gs.drawReveal.needsDecision&&(
        <DrawRevealModal
      ...
      {!suppressAnim&&phase==='TREASURE_AOE_DODGE_DECISION'&&gs.drawReveal&&myTurn&&(
        <TreasureDodgeModal
          drawReveal={gs.drawReveal}
          onRoll={handleTreasureAOEDodgeRoll}
          onSkip={handleTreasureAOEDodgeSkip}
          thinkingText={gs._isMP&&!myTurn?`其他玩家思考中…`:''}
        />
      )}
      {/* Other players see thinking text during AOE dodge */}
      {!suppressAnim&&phase==='TREASURE_AOE_DODGE_DECISION'&&gs.drawReveal&&!myTurn&&gs._isMP&&(
        <div style={{
          position: 'fixed',
          top: '50%',
          left: '50%',
      ...
      {/* 灵龟卜祝选择界面 */}
      {!suppressAnim&&phase==='TORTOISE_ORACLE_SELECT'&&gs.abilityData&&myTurn&&(
        <div style={{position:'fixed',inset:0,display:'flex',alignItems:'center',justifyContent:'center',zIndex:400}}>
          <div style={{background:'#150e07dd',border:'2px solid #a78bfa',boxShadow:'0 0 60px #a78bfa44, 0 0 120px #000a',borderRadius:4,padding:'20px 28px',maxWidth:320,width:'90%',textAlign:'center'}}>
            <div style={{fontFamily:"'Cinzel',serif",color:'#a78bfa',fontSize:16,letterSpacing:2,marginBottom:16}}>── 灵龟卜祝 ──</div>
            <div style={{fontFamily:"'IM Fell English','Georgia',serif",fontStyle:'italic',color:'#b09090',fontSize:14,marginBottom:20,lineHeight:1.4}}>请选择你手中最多的一个字母或数字编号</div>
            <div style={{display:'flex',gap:10,flexWrap:'wrap',justifyContent:'center',marginBottom:20}}>
              {gs.abilityData?.selectableKeys?.map((key,i)=>(
                <button key={i} onClick={()=>tortoiseOracleSelect(key)} style={{
